...
public class TicTacToe extends JFrame
{
   ...
   private TicTacToeAIAlternate ai;
   private int aiMove;
   private Random ran;
   private int aiType;
   /**
   ...
   public class startListener implements ActionListener
   {
      ...
      public void actionPerformed(ActionEvent start)
      {
         ...
         aiType = ran.nextInt(2);
         turn = 0;
         if(aiType == 0)
         {
	            aiMove = ai.getNextMove(ticTacArray,turn);
	            tiles[aiMove].setText(marker);
	            ticTacArray[aiMove] = marker;
	            tiles[aiMove].setEnabled(false);
	            System.out.println(ticTacArray[aiMove]+ " ");
	            turn++;
         }
      ...
      public void actionPerformed(ActionEvent press)
      {
         for(int j = 0; j < 10; j++)
         {
            if(press.getSource() == tiles[j])
            {
               ...
               if(turn%2 == 0)
               {
                  status.setText("It is Team O's turn "+turn);
               }
               else
               {
                  status.setText("It is Team X's turn"+turn);
               }
               if(aiType == 1)
               {
                  marker = "X";
